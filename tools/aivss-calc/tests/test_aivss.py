...
from aivss_calc.decision import TIMELINE_URGENCY, decide, escalate
...
class TestIdentityRule:
    ...
    def test_td_alone_does_not_change_the_score(self):
        """TD (Agent Untraceability) is mandatory but must never affect the numeric score."""
        metrics = parse_cvss_vector(EXAMPLE_VECTOR)
        for td in ("H", "M", "L"):
            profile = AIProfile(td=td, scored_present=False)
            assert profile.effect_class() == "A0"
            assert lookup_aivss(EXAMPLE_VECTOR, metrics, "A0")["aivss_btea"] == EXAMPLE_CVSS_BTE
...
class TestEndToEnd:
    ...
    def test_mode1_equals_cvss_bte(self):
    ...
    def test_mode2_is_provisional_and_higher(self):
        report = self._assessment()
        assert report["scores"]["mode2_macrovector"]["aivss_btea"] == 9.0
    ...
    def test_td_only_yields_identity_and_a0(self):
        ...
        assert report["scores"]["mode2_macrovector"]["aivss_btea"] == EXAMPLE_CVSS_BTE
        assert report["scores"]["mode1_interpretation"]["aivss"] == EXAMPLE_CVSS_BTE
